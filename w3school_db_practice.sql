...
-- Optional: Uncomment these lines if you need to clean up existing tables first
...
-- --------------------------------------------------------
-- Foreign Key Indexes for optimization
-- --------------------------------------------------------
...
-- --------------------------------------------------------
-- Sequence adjustments for SERIAL auto-increments
-- --------------------------------------------------------
...
SELECT setval(pg_get_serial_sequence('employees', 'employeeid'), COALESCE(max(employeeid), 1)) FROM employees;
SELECT setval(pg_get_serial_sequence('shippers', 'shipperid'), COALESCE(max(shipperid), 1)) FROM shippers;
SELECT setval(pg_get_serial_sequence('suppliers', 'supplierid'), COALESCE(max(supplierid), 1)) FROM suppliers;
...
SELECT setval(pg_get_serial_sequence('order_details', 'orderdetailid'), COALESCE(max(orderdetailid), 1)) FROM order_details;
